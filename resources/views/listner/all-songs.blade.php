    ...
    <div class="container mx-auto px-6">
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-3">
                <h1 class="text-3xl font-bold text-white">All Songs</h1>
                <span class="px-2 py-1 text-xs bg-red-600/20 text-red-500 rounded-full">{{ $songs->total() }}</span>
            </div>
            
            <!-- Filter/Sort Options (optional) -->
            <div class="flex items-center gap-4">
                <select class="bg-zinc-900 text-white text-sm rounded-lg px-4 py-2 border border-zinc-800">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-5">
            @foreach($songs as $track)
                <div class="group">
                    <!-- Track Cover -->
                    <div class="relative aspect-square mb-3 bg-zinc-900 border border-zinc-800 rounded-md overflow-hidden shadow-lg">
                        <!-- Cover Image -->
                        ...
                        <div class="absolute inset-0 flex items-center justify-center">
                            <button onclick="playTrack('{{ asset('storage/' . $track->audio_file) }}', '{{ $track->title }}', '{{ $track->user->name }}', '{{ asset('storage/' . $track->cover_image) }}')"
                        ...
                        @if(isset($track->duration))
                            <div class="absolute bottom-2 right-2 bg-black/60 backdrop-blur-sm text-white text-xs px-2 py-0.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                {{ $track->duration }}
                            </div>
                        @endif
                    </div>
